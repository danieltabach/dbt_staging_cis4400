...
boroughs as (SELECT CAST(Zip AS STRING) AS Zip, Borough FROM `data-project-329222.newyork_311.Boroughs`),

finalborough as (SELECT Borough,zipcode FROM final INNER JOIN boroughs ON Zip = zipcode)

SELECT distinct {{ dbt_utils.surrogate_key(['Borough', 'zipcode']) }} as location_dim_id,Borough,zipcode 
FROM finalborough